	...
	for _, bindName in ipairs(BindNames) do
			...
			CurrentBinds[bindName] = rom.inputs.on_key_pressed({
				...
				function()
					...
					if GrowUnstuck then
						thread(
							InCombatTextArgs,
							{ TargetId = CurrentRun.Hero.ObjectId, Text = "UnstuckEnablePopUp", PreDelay = 0, Duration = 2.5, Cooldown = 1.0 }
						)
					else
						thread(
							InCombatTextArgs,
		...
		elseif bindName == "bigger" then
			CurrentBinds[bindName] = rom.inputs.on_key_pressed({
				...
				function()
					if config.sizeControlInRuns == true or (CurrentHubRoom ~= nil and config.sizeControl == true) then
						if CurrentRun and CurrentRun.Hero and HeroHasTrait("HealthGrowTrait") then
							AddMaxHealth(10, nil)
						end
